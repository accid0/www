...
use packages\models\db\LazyPDO;
...
//define('XPDO_CORE_PATH', "packages/models/db/xpdo-2.1.3-pl/");
...
class xPDO {
    ...
    public function connect($driverOptions= array ()) {
        if ($this->pdo === null) {
            ...
            try {
                $this->pdo= new LazyPDO($this->config['dsn'], $this->config['username'], $this->config['password'], $this->config['driverOptions']);
    ...
    public function getOption($key, $options = null, $default = null) {
        ...
        if (is_array($key)) {
            ...
            if (!is_null($options) && array_key_exists($key, $options)) {
                $option= $options[$key];
