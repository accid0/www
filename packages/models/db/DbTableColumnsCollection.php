...
class DbTableColumnsCollection implements Iterator {
  private $position = 0;
  private $keys = array();
  /**
   * 
  ...
  private function init( $array){
    ...
    $this->ensure( is_null( $this->table) || !is_array( $fields) || empty($fields),
    	"[DbTableCoumnsCollection]: не правильно задан массив [" .
  ...
  function __get( $key){
    ...
    if ( $this->fields[$key] instanceof  self) return $this->fields[$key];
    else return $this->table . "." . $this->fields[$key];
  }
  /**
   * 
   * Enter description here ...
  ...
  function current(){
    ...
    return $this->table . "." . $this->fields[ $this->keys[ $this->position]];
  }
  /**
   * @return int
   * Enter description here ...
